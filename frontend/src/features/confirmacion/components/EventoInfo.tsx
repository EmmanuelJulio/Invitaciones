...
export const EventoInfo: React.FC<EventoInfoProps> = ({ evento }) => {
  return (
    <div className="card space-y-6 fade-in">
      ...
      <div className="bg-white/20 rounded-xl p-4 sm:p-6 space-y-3">
        <div className="flex items-center space-x-3">
          ...
          <span className="font-semibold text-white text-sm sm:text-base">Código de Vestimenta:</span>
        </div>
        <p className="text-white text-sm sm:text-base ml-0 sm:ml-8 font-medium">{evento.codigoVestimenta}</p>
      </div>

      {evento.notaEspecial && (
        <div className="bg-amber-900/30 border-l-4 border-amber-300 p-4 sm:p-6 rounded-r-xl">
          <div className="flex items-start space-x-3 mb-3">
            ...
            <span className="font-semibold text-amber-100 text-sm sm:text-base">Nota Importante:</span>
          </div>
          <p className="text-amber-200 text-sm sm:text-base ml-0 sm:ml-8 leading-relaxed">{evento.notaEspecial}</p>
        </div>
      )}
    </div>
  );
